...
namespace Test.Tremendous1192.SelfEmployed.CoMPASS.Books
{
    public partial class BasicOfStatisticalDataAnalysis
    {
        ...
        public void Exercise_2_1()
        {
            ...
            double[,] designMatrix_y = new double[5, 1];
            for (int j = 0; j < design_Matrix_x.GetLength(0); j++)
            {
                designMatrix_y[j, 0] = Math.Round(10 * (design_Matrix_x[j, 0] - 6));
                Console.WriteLine("y" + (j + 1) + "\t" + designMatrix_y[j, 0]);
            }
            Console.WriteLine("");

            Console.WriteLine("(b) 上記の5つの数値について,平均,平方和,分散,標準偏差を求めなさい.");
            double[,] summary = Statistics<double>.Summary(designMatrix_y);
            ...
            for (int k = 0; k < summary.GetLength(1); k++)
            {
                Console.Write(summary[5, k] / 10 + 6 + "\t");
            }
            Console.WriteLine("");
