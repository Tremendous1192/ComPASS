...
namespace Test.Tremendous1192.SelfEmployed.CoMPASS.MathApp
{
    public partial class MatrixTest
    {
        ...
        public void SubtractDecimalTest()
        {
            ...
            Console.WriteLine("行列の差のテストメソッド");
            Matrix<decimal> matrix_A = new Matrix<decimal>(2, 3);
            Matrix<decimal> matrix_B = new Matrix<decimal>(2, 3);
            for (int i = 0; i < 2; i++)
            {
                for (int j = 0; j < 3; j++)
                {
                    matrix_A[i, j] = i + j + 0.3m;
                    matrix_B[i, j] = i * 3 + j * 2 + 0.3m;
                }
            }
            ...
            var matrix_C = matrix_A.Subtract(matrix_B);
            Console.WriteLine("\n行列 C = A - B");
            ...
            var matrix_D = Matrix<decimal>.Subtract(matrix_A, matrix_C);
            Console.WriteLine("\n行列 D = A - C");
            ...
            var matrix_E = matrix_B - matrix_C;
            Console.WriteLine("\n行列 E = B - C");
